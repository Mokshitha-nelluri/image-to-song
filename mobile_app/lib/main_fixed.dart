...
class _HomePageState extends State<HomePage> {
  ...
  Future<void> _checkBackendConnection() async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        setState(() {
          _isSpotifyAuthorized = data['features']?['spotify_oauth'] ?? false;
        });
  ...
  Future<void> _authorizeSpotify() async {
    ...
    try {
      ...
      _addDebugInfo('Spotify login response: ${response.statusCode}');
      _addDebugInfo('Spotify login data: ${response.body}');

      if (response.statusCode == 200) {
        final data = json.decode(response.body);
        final authUrl = data['auth_url'];

        _addDebugInfo('Auth URL: $authUrl');
                ...
                child: Column(
                  children: [
                    Icon(
                      Icons.music_note,
                      size: 48,
                      color: _isSpotifyAuthorized ? Colors.green : Colors.grey,
                    ),
        ...
        .map(
          (track) => Card(
            ...
            child: ListTile(
              ...
              subtitle: Text(
                '${track['artist']} • Popularity: ${track['popularity']}/100',
              ),
              trailing: IconButton(
                icon: const Icon(Icons.open_in_new),
                onPressed: () => _openSpotifyTrack(track['spotify_url']),
              ),
            ),
          ),
        )
